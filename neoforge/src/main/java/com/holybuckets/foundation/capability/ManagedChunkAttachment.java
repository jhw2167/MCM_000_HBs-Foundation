...
/**
 * Description: ManagedChunk persistence via Balm's data attachment API (NeoForge only).
 *
 * <p>Previously this used a NeoForge {@code DeferredRegister<AttachmentType>} plus a manual
 * {@code LevelCallback.Chunk.LOAD} listener registered in the {@code @Mod} constructor, which
 * crashed at runtime ("LevelCallback.Chunk.LOAD unbound") because Balm only binds its event
 * mappings during {@code Balm.initializeMod(...)}. Balm's data attachment API registers through
 * that same (bound) init, so both the registration and the load hook are safe here.
 *
 * <p>Design (per author decision): the common {@link ManagedChunk} creation flow
 * ({@code ManagedChunkEvents.onChunkLoad}) is unchanged — it still creates ManagedChunks into
 * {@code ManagedChunkUtility}'s registry. Balm is used purely for <b>persistence</b>: the created
 * ManagedChunk is attached to the chunk so Balm saves it, and Balm auto-decodes it back on load
 * (whereupon the {@code ManagedChunk(CompoundTag)} constructor re-registers it into the registry).
 */
